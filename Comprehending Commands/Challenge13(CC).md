...
If you don't specify a search location, find uses the current working directory (.). For example:
...
These will cause find to generate errors, but you can ignore those; we won't hide the flag there! Finally, find can take a while; be patient!

### Solve
**Flag:** `pwn.college{0XKLBNdCXficuHbnLyG83tVHGkk.QXyMDO0wCM5AzNzEzW}`
I just used the find command to list all the directories where the flag file can be, and then tried all the directories for the actual flag file.

### New Learnings
I learnt to use the find command for the flag
